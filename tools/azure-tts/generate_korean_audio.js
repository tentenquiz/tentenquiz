...
const LANGUAGE = { code: 'ko', suffix: 'ko', label: '한국어', locale: 'ko-KR' };
const VOICE_NAME = 'ko-KR-SunHiNeural';
const RATE = '-12%';
const SPEED_PERCENT = 88;
const OUTPUT_ROOT = path.join(PROJECT_ROOT, 'audio', 'ko');
const REVIEW_ROOT = path.resolve(PROJECT_ROOT, '..', 'tts-review', 'korean-audio-88');
const OVERRIDE_FILE = path.join(__dirname, 'pronunciation-overrides.ko.json');
...
function loadOverrides() {
    ...
    if (payload.locale !== LANGUAGE.locale || payload.voice !== VOICE_NAME) {
        throw new Error('한국어 발음 재정의 파일의 locale 또는 voice가 현재 설정과 다릅니다.');
    }
...
function loadSourceData(overrides) {
    ...
    for (const fileName of DATA_FILES) {
        ...
        rows.forEach((row, rowIndex) => {
            ...
            const displayText = String(row.word_ko || '').trim().normalize('NFC');
            const readingText = String(row.reading_ko || displayText).trim().normalize('NFC');
            const override = overrides[id] || null;
            const speechText = String(override ? override.speechText : readingText).trim().normalize('NFC');
            const webAudioFile = `audio/ko/${section}/${id}.mp3`;
            ...
            if (!displayText || !readingText || !speechText) throw new Error(`${id}의 한국어 음성 입력이 비어 있습니다.`);
            if (displayText !== readingText) throw new Error(`${id}의 word_ko와 reading_ko가 다릅니다.`);
            if (/[\\/:*?"<>|]/.test(speechText)) throw new Error(`${id}의 음성 입력에 허용하지 않는 문자가 있습니다.`);
            if (row.audioFile_ko && row.audioFile_ko !== webAudioFile) {
                throw new Error(`${id}의 기존 audioFile_ko 경로가 규칙과 다릅니다: ${row.audioFile_ko}`);
            }
    ...
    if (jobs.length !== 2500) throw new Error(`한국어 생성 대상은 2,500개여야 합니다. 현재: ${jobs.length}`);
    return { sources, jobs };
}
...
function addRiskFlags(jobs) {
    ...
    for (const job of jobs) {
        ...
        if (meanings.size > 1) reasons.push('같은 한국어 표기·다른 의미');
        if (/^[가-힣]$/.test(job.readingText)) reasons.push('한 음절 표제어');
        if (job.readingText.split(/\s+/).length >= 3) reasons.push('세 단어 이상 띄어쓰기');
        if (job.readingText.length >= 9) reasons.push('긴 표제어');
...
function commitAudioPaths(sources, jobs) {
    const backupRoot = path.join(PROJECT_ROOT, 'backups', `before-ko-audio-${makeTimestamp()}`);
    ...
    for (const source of sources) {
        ...
        for (const job of fileJobs) source.rows[job.rowIndex].audioFile_ko = job.webAudioFile;
...
<head>
    ...
    <title>TentenQuiz 한국어 음성 전수 검수</title>
    <style>
...
<main>
    <section class="hero">
        <h1>한국어 음성 전수 검수</h1>
        <p>단어와 뜻을 확인하며 듣고 <b>정상</b> 또는 <b>재생성 필요</b>를 표시하세요. 위험 후보부터 먼저 들을 수 있습니다.</p>
...
(() => {
    ...
    const storageKey = 'tentenquiz.koAudioReview.v1';
    const pageSize = 25;
    let page = 1;
    ...
    document.getElementById('export').addEventListener('click', () => {
        ...
        link.download = 'tentenquiz-ko-audio-review.json'; link.click(); setTimeout(() => URL.revokeObjectURL(link.href), 1000);
    });
    render();
})();
</script>
...
function writeReviewPackage(settings, jobs, sources, generationStats, packageAudio) {
    ...
    const items = jobs.map((job) => ({
        ...
        reviewAudioFile: `audio/ko/${job.section}/${job.id}.mp3`
    }));
    const manifest = {
        version: 1,
    ...
    fs.writeFileSync(path.join(REVIEW_ROOT, 'pronunciation-risk-ko.json'), `${JSON.stringify(riskPayload, null, 2)}\n`, 'utf8');
    fs.writeFileSync(path.join(REVIEW_ROOT, 'index.html'), createReviewHtml(manifest), 'utf8');

    const csvRows = [['id', 'section', 'stage', 'word_ko', 'note_ko', 'risk_reasons', 'audio_file']];
    items.forEach((item) => csvRows.push([
        item.id, item.section, item.stage, item.displayText, item.noteKo, item.riskReasons.join(' | '), item.reviewAudioFile
    ]));
    const csv = `\uFEFF${csvRows.map((row) => row.map(csvEscape).join(',')).join('\r\n')}\r\n`;
    fs.writeFileSync(path.join(REVIEW_ROOT, 'pronunciation-review-ko.csv'), csv, 'utf8');

    if (packageAudio) {
        for (const job of jobs) {
            const reviewFile = path.join(REVIEW_ROOT, 'audio', 'ko', job.section, `${job.id}.mp3`);
...
async function main() {
    ...
    console.log(`한국어 대상 ${jobs.length}개, ${totalCharacters}자, 여성 음성 ${VOICE_NAME}, 속도 ${SPEED_PERCENT}%`);
    ...
    if (args.commitData) {
        ...
        console.log(`한국어 audioFile_ko 2,500개 연결 완료. 백업: ${backupRoot}`);
    }
